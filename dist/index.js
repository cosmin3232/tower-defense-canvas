...
const game = new Game();
const activeTower = Math.floor(Math.random() * allDefenders.length);
let chooseTower;
...
const handleEnemies = () => {
    for (let i = 0; i < config.enemies.length; i++) {
        ...
        if (config.enemies[i].health <= 0) {
            const gainedResources = config.enemies[i].maxHealth / 10;
            ...
            const findThisIndex = config.enemiesPositions.indexOf(config.enemies[i].y);
    ...
    if (config.frame % config.enemiesInterval === 0 && config.score < config.winningScore && config.enemies.length < 10) {
        const verticalPosition = Math.floor(Math.random() * 5 + 1) * config.cellSize + config.cellGap;
        const randomArray = Math.floor(Math.random() * allEnemies.length);
        config.enemies.push(new Enemy(230, 750, allEnemies[randomArray].type, allEnemies[randomArray].name, allEnemies[randomArray].health, allEnemies[randomArray].speed, allEnemies[randomArray].imagePositions));
